...
import { useGetAuth } from '../layout-dashboard';

export default component$(() => {
  const loc = useLocation();

  const { value: { guilds } } = useGetAuth();
    ...
    <section class="mx-auto max-w-screen-2xl px-6 pt-12 items-center" style={{ minHeight: 'calc(100vh - 64px)' }}>
      ...
      <div class="flex flex-wrap justify-center sm:justify-evenly gap-5 my-12">
        {
          ...
          store.GuildList.filter(guild => guild.mutual).map(guild => {
            return (
              <Link key={guild.id} href={`/dashboard/${guild.id}/general`} class="flex flex-col items-center relative rounded-xl group sm:hover:-translate-y-4 hover:scale-105 transition-all duration-200 w-14 sm:w-48 py-10 pic-link">
      ...
      <div class="text-center sm:hidden">
        ...
        <p class="mt-5 text-xl sm:text-2xl md:text-3xl text-gray-500">
          These servers don't have Basement Bot yet! Select a server to invite Basement Bot to.
        </p>
      </div>
      <div class="flex flex-wrap justify-center sm:justify-evenly gap-5 my-12">
        {
...
export const head: DocumentHead = {
  ...
  meta: [
    {
      name: 'description',
      content: 'The Basement Bot Dashboard',
    },
    {
